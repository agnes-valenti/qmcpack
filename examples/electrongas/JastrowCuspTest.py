...
class wf:
    ...
    def set_cusp_parallel(self):
        coefs_=self.coefs[0]
        coefs_[0]=coefs_[2]+2.0*self.DeltaRu*self.e_squared*self.mstar/3.0
        self.coefs[0]=coefs_

    def set_cusp_antiparallel(self):
        coefs_=self.coefs[0]
        coefs_[0]=coefs_[2]+2.0*self.DeltaRu*self.e_squared*self.mstar/1.0
        self.coefs[0]=coefs_


    def loadcoefs(self,components=1):
    ...
    def loadcoefs_forderiv(self,igderiv,ideriv,delta=1e-4,components=1):
        self.NumParamsu=self.NumParams-self.NumParamsa2-self.NumParamsb2-self.NumParamsx2-self.NumParamsy2-1
        self.numCoefs=self.NumParamsu+3+self.NumParamsa2+3+self.NumParamsb2+3+self.NumParamsx2+3+self.NumParamsy2+3;
        Parameters=[]
        for ig in range(components):
    ...
    def load_U(self):
       ...
       self.rvalues=np.zeros(Nvalues1+1+Nvalues2+1);
       self.Uvalues=np.zeros(Nvalues1+1+Nvalues2+1);

  
       self.rvalues[:Nvalues1+1]=uscr1[0,:];
       self.Uvalues[:Nvalues1+1]=uscr1[1,:]
  
       self.rvalues[Nvalues1+1:]=uscr2[0,:];
       self.Uvalues[Nvalues1+1:]=uscr2[1,:]


 
       self.Uvalues=self.Uvalues*4.0/self.d_;
    ...
    def get_index(self,rvalue):
       if (rvalue<=1):
          ...
          return (min(int((rvalue-1)*Nvalues2/5000-1+(Nvalues1+1)),Nvalues1+Nvalues2));
    ...
    def get_linear_interpolated_U(self,rvalue):
       ...
       return_U=Uvalues[0]*self.rvalues[0]/rvalue;
    ...
    def Eloc_finitediff_2particles_relativecoords(self,r12,rp12,h=1e-5):
        ...
        #print("psi: ",psi)
        return 0.5*self.e_squared/np.sqrt(np.dot(r12,r12)) +Ekin/psi
...
class TestMain:
    ...
    def get_Ekinfinitediff(self):
        ...
        for i in range(100):
            ...
            x_[1,:]=-0.5*r12+rp12
            xvalues.append(x_)
            print(Wf_.Ekinloc_finitediff(xvalues,1e-4),Wf_.Eloc_finitediff_2particles_relativecoords(r12,rp12,1e-4))
