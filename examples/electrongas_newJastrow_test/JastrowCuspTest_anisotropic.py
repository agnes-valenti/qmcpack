...
class TestMain:
    ...
    def get_Ekinfinitediff(self):
        ...
        for i in range(601):
            ...
            xplot=[x1]
            Enmean=0
            for phi_i in range(16):
                x1new=np.copy(x1)
                r12abs=np.linalg.norm(r12)
                re12=r12/r12abs
                phi=np.arccos(-re12[0])
                #print("phi diff: ",phi-np.arcsin(re12[1]))
                r12new=r12abs*np.array([np.cos(phi+phi_i*np.pi/8.0),np.sin(phi+phi_i*np.pi/8.0)])
                x1new[0,:]=r12new+x1[1,:]
                xvalues=[x1new,x2_]
                xplot.append(x1new)
                #print("E phi rotated: ",(phi_i*np.pi/4.0)/(2*np.pi),Wf_.Ekinloc_finitediff(xvalues,1e-4)) #,Wf_.Eloc_finitediff_2particles_relativecoords(r12,rp12,1e-4))
                Enmean+=Wf_.Ekinloc_finitediff(xvalues,1e-4)
            print("Enmean: ",Enmean/16.0)
            #plt.figure()
