...
def get_P(Etatilde_):
    ...
    for i in range(Nkx):
       for j in range(Nky):
           ...
           ky_=Ky[j,i]
           if P[k_it]<0.5:
              if np.heaviside(kF**2-Etatilde_**(tau/2.0)*kx_**2- Etatilde_**(-tau/2.0)*ky_**2,1)>0.5:
                P[k_it]=1
...
etatildeValues=np.linspace(1.0,5.79,nEtas)
#etatildeValues=np.linspace(2.0,2.001,nEtas)

for eta_it, etatilde_ in enumerate(etatildeValues):

    Etatilde=1.0/etatilde_ #5.79 #1.0 #1.0/5.4 #5.79 #4.013381993233915   #5
