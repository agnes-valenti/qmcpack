...
nparticles_=np.arange(100,120,2)
nparticles_=np.array([4])
NumOffsets=1 #41 #200 #21


factordeg=1
sym_ind=2 #1: sym, 2: vp/sp, 3: svp
vp=False #True
sp=True #True #False
if sym_ind==2:
   factordeg=2
...
def make_EA():
   ...
   E_=np.zeros((Nkx*Nky,2))
   A=np.zeros((Nkx*Nky,6,4))+0j
   E2_=np.zeros((Nkx*Nky,2))
       ...
       for j in range(Nky):
           for Tau in range(2):
               for s in range(2):
                   ...
                   kx_=Kx[j,i]
                   ky_=Ky[j,i]
                   E_[k_it,Tau]=Eta**(0.5*tau)/(2.0*mstar)*kx_**2+Eta**(-0.5*tau)/(2.0*mstar)*ky_**2
                   #print(E[k_it,Tau])
           k_it+=1
   return E_
...
def sort_notfilledkit(deltaK):
    
    sortedEn=np.zeros((Nkx*Nky))
    sortedEnhelper=np.zeros((Nkx*Nky))
    ...
    for i in range(Nkx):
      for j in range(Nky):
        ...
        ky_=Ky[j,i]
        tau=1
        en=Etatilde**(0.5*tau)/(2.0*mstar)*kx_**2+Etatilde**(-0.5*tau)/(2.0*mstar)*ky_**2
        sortedEn[k_it]=en
        sortedEnhelper[k_it]=en -1e-4*deltaK**2*(Etatilde**(0.5*tau)+Etatilde**(-0.5*tau))/(2.0*mstar)*(np.sign(kx_)*np.sign(ky_)+0.01*np.sign(kx_)+0.02*np.sign(ky_))
        sortedkit[k_it]=int(k_it)
        k_it+=1

    #sortedEn[sortedEn==0]=2*np.max(sortedEn)

    c=np.argsort(sortedEnhelper)
    sortedkit=(sortedkit[c])
    ...
    return sortedkit,sortedEn

def sort_notfilledkit2(deltaK):
...
def get_P(Etatilde_,deltaK=0):
    sortedkit,sortedEn=sort_notfilledkit(deltaK)
    indices=sortedkit[:int(np.round(Nparticles/factordeg))]
    P=np.zeros((Nkx*Nky))+0j
    P[indices]=1
    return P

def get_P2(Etatilde_,deltaK=0):
    sortedkit,sortedEn=sort_notfilledkit2(deltaK)
    indices=sortedkit[:int(np.round(Nparticles/factordeg))]
    P=np.zeros((Nkx*Nky))+0j
    P[indices]=1
    return P
...
def get_P2(Etatilde_,deltaK=0):
    ...
    for mu_it, mu in enumerate(muvalues):
       ...
       if diff_dens<diff_dens_old:
          ...
          diff_dens_old=diff_dens
       #P_=np.copy(P)
    print("Nparticles: ",Nparticles, "sum P2: ", factordeg*np.sum(P_))
    return P_
...
def get_energy(P_matrix,P_matrix2,Tnew,Tnew2,n0_):
    energy=0
    energy1=0
    energy2=0
    dens=0
    ...
    for k_it in range(Nkx*Nky):
        if sym_ind==1:
           energy+=2*E[k_it,0]*P_matrix[k_it] + 2*E2[k_it,0]*P_matrix2[k_it]           #k -> -k: TR symmetry (?)
           #print("E:",E[k_it,0],E[k_it2,1])
           dens+=2*P_matrix[k_it]/Area+2*P_matrix2[k_it]/Area
           energy+=0.5*2*P_matrix[k_it]*Tnew[k_it]+0.5*2*P_matrix2[k_it]*Tnew2[k_it]
           interaction+=0.5*2*P_matrix[k_it]*Tnew[k_it]+0.5*2*P_matrix2[k_it]*Tnew2[k_it]
        elif sym_ind==2 and vp:
        ...
        elif sym_ind==2 and sp:
           energy+=E[k_it,0]*P_matrix[k_it]+ E2[k_it,0]*P_matrix2[k_it]          #k -> -k: TR symmetry (?)
           energy1+=E[k_it,0]*P_matrix[k_it]
           energy2+=E2[k_it,0]*P_matrix2[k_it]
           #print("E:",E[k_it,0],E[k_it2,1])
           dens+=P_matrix[k_it]/Area+P_matrix2[k_it]/Area
           energy+=0.5*P_matrix[k_it]*Tnew[k_it]+0.5*P_matrix2[k_it]*Tnew2[k_it]
           interaction+=0.5*P_matrix[k_it]*Tnew[k_it]+0.5*P_matrix2[k_it]*Tnew2[k_it]
        elif sym_ind==3:
...
for kit,nparticles in enumerate(nparticles_):
    ...
    kFmaxvalue=int(kFmaxvalue/deltaK)*deltaK
    ...
    for offset_x in kxoffsets:
        ...
        k1x=np.arange(-kFmaxvalue,kFmaxvalue,deltaK)+offset_x
        ...
        P2=get_P2(Etatilde,deltaK) 
        #print("Etatilde: ",Etatilde)
        #print("P: ",P)
        Tnew=HF_step(P,n1)
        Tnew2=HF_step(P2,n1)
        Energy, dens, interaction,hartree=get_energy(P,P2,Tnew,Tnew2,n1)
        #print("Energy, dens, interaction, hartree: ", Energy, dens, interaction, hartree)
        Nspins=np.sum(P) + np.sum(P2)
...
thetax=kxoffsets[nkxmin] #kxoffsets[nkxmin] #factors[nfactor]*kxoffsets[nkxmin]
k_=np.zeros(5)
k_[0]=kfStartArray[n]
k_[1]=dKs[n]
k_[2]=NkxArray[n]
...
k1y=np.arange(-kFmaxvalue,kFmaxvalue,deltaK)+thetay
k1x=np.arange(-kFmaxvalue,kFmaxvalue,deltaK)+thetax
...
el=patches.Ellipse((0,0), kxF_*2, kyF_*2, angle=0, fill=False, zorder=2)
el2=patches.Ellipse((0,0), kyF_*2, kxF_*2, angle=0, fill=False, zorder=2, color='red')
ax.add_patch(el)
ax.add_patch(el2)
for i in range(int(NkxArray[n])):
    for j in range(int(NkxArray[n])):
        kx_=Kx[j,i]
        ky_=Ky[j,i]
        if Pmatricessp[n][k_it,0]>0.5:
           print("0","kx: ",kx_,"ky_: ",ky_)
           ax.scatter(kx_,ky_,marker='^',c='blue')
        if Pmatricessp[n][k_it,2]>0.5:
           ax.scatter(kx_,ky_,marker='v',c='yellow')
           print("2","kx: ",kx_,"ky_: ",ky_)
        k_it+=1
