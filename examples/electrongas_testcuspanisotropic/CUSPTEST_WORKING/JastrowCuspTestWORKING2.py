        ...
        for ig in range(self.ngroups):
            for iat in range(self.nspins[ig]):
                ...
                for jat in range(iat):
                    ...
                    U+=self.get_linear_interpolated_U(r)*self.e_squared  #self.e_squared/r
...
class TestMain:
    ...
    def get_Ekinfinitediff(self):
        ...
        xvalues=[]
        x_=np.zeros((2,2))
        x_[0,:]=np.array([3.1238295408e+01,  4.2012193946e+01])
        x_[1,:]=np.array([9.5666709302e+01,  8.4170479936e+01])
        xvalues.append(x_)

        psi2=Wf_.Psi(xvalues)
        print(psi2)

        deltaR=np.array([[1.073806900e-01,  7.026380998e-02],[0,0]])
        r=np.linspace(0,0.01,100)*self.L
        for i in range(601):
            #r12=np.array([r[100-1-i],0.0])
            xvalues=[]
            xvalues.append(x_+i*deltaR)
            print(xvalues)
