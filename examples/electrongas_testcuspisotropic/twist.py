...
def get_P(Etatilde_,deltaK=0):
    ...
    for mu_it, mu in enumerate(muvalues):
       ...
       if diff_dens<diff_dens_old:
          ...
          #print("mu diff: ",mu-kF,np.sum(P)*factordeg,n1*area) 
    return P_
...
etatildeValue=1.7609294853520199
if sym_ind==1:
...
NumOffsets=10
nEtas=(2*NumOffsets+1)**2
kxoffsets=np.linspace(-deltaK_,deltaK_,2*NumOffsets+1)
...
for offset_x in kxoffsets:
  for offset_y in kyoffsets: 
    ...
    k1y=np.linspace(-2*kFmax+offset_y,2*kFmax+offset_y,Nky)
