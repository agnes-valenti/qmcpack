...
EkinInf=4.9616599741533115*1e-23  #4.1963*1e-23 #  4.9616599741533115*1e-23  #4.1963*1e-23 #4.9616599741533115*1e-23
...
kxoffsets=np.linspace(0.0,deltaK_*0.5,2*NumOffsets+1)
...
for offset_x in kxoffsets:
  for offset_y in kyoffsets: 
    ...
    k1x=np.linspace(-2*kFmax+1.0*offset_x,2*kFmax+1.0*offset_x,Nkx)
    k1y=np.linspace(-2*kFmax+1.0*offset_x,2*kFmax+1.0*offset_x,Nky)
...
print(numberspins)
#n=np.argmin(np.abs(EnergiesSI-EnergyInf))
...
print("Ex min SI: ",ExsSI[n], "Ex: ",Exs[n])
...
plt.plot(kxoffsets,EkinsSI)
plt.hlines(EkinInf,np.min(kxoffsets),np.max(kxoffsets))
plt.vlines(kxoffsets[n],np.min(EkinsSI),np.max(EkinsSI),color='red')
plt.show()
