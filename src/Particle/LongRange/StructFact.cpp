...
{
  //std::cout<<"AV entering StructFact::updateAllPart"<<std::endl;
  ScopedTimer local(update_all_timer_);
  computeRhok(P);
  //std::cout<<"AV exiting StructFact::updateAllPart"<<std::endl<<std::endl;
}
...
/** evaluate rok per species, eikr  per particle (rhok=e^ikr, used to compute Structure factor)
 */
...
#else
      for (int ki = 0; ki < nk; ki++){
        //std:: cout<<"ki: "<<ki<<" kpts_cart: "<<k_lists_->kpts_cart[ki]<<std::endl;
