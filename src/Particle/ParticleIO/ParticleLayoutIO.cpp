...
bool LatticeParser::put(xmlNodePtr cur)
{
  ...
  if (rs > 0.0)
  {
    ...
    {
      if (nsh > 0)
        nptcl = 11; //AV set to dummy value - variable used?? 2 * heg.getNumberOfKpoints(nsh);
      else
        nsh = 1; //AV set to dummy value - variable used??, heg.getShellIndex(nptcl / 2);
    }
    else
    { //             spin polarized
      if (nsh > 0)
        nptcl = 11; //AV set to dummy value - variable used?? heg.getNumberOfKpoints(nsh);
      else
        nsh = 1; //AV set to dummy value - variable used?? heg.getShellIndex(nptcl);
    }
    ParticleLayout_t::Scalar_t acubic = heg.getCellLength(rs); //1; //AV set to dummy value - variable used?? heg.getCellLength(nptcl, rs);
    std::cout<<" AV in ParticleLayoutIO::put, acubic: " <<acubic<<" *rs: " <<acubic*rs<<std::endl;
  ...
  if (lattice_defined)
  {
    std::cout<<"AV in ParticleLayoutIO, lattice defined"<<std::endl;
    lattice_in *= a0;
    ref_.set(lattice_in);
  }
  if (ref_.SuperCellEnum == SUPERCELL_OPEN){
    std::cout<<"AV in ParticleLayoutIO, supercell open"<<std::endl;
