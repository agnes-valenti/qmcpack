...
#endif
      << "\n  Blub3 Total number of MPI ranks = " << OHMMS::Controller->size()
...
//omp_set_num_threads(1);  //AV -> change!!! (Remove line)
...
#pragma omp parallel
  {
    ...
    if (L1_tid == 0){
      std::cout << "(AV)  OMP 1st level threads     = " << omp_get_num_threads() << std::endl;
      std::flush(std::cout);}
#pragma omp parallel
    {
      ...
      {
        if (L2_num_threads == 1)
          std::cout << "  OMP nested threading disabled or only 1 thread on the 2nd level" << std::endl;
        else
          std::cout << "  OMP 2nd level threads     = " << L2_num_threads << std::endl;
      }
    }
  }
  ...
  std::cout << "  OpenMP target offload to accelerators build option is enabled" << std::endl;
#endif
#if defined(ENABLE_CUDA) || defined(QMC_CUDA)
  std::cout << "  CUDA acceleration build option is enabled" << std::endl;
#endif
#if defined(ENABLE_ROCM)
  std::cout << "  ROCM acceleration build option is enabled" << std::endl;
#endif
#endif
#ifdef ENABLE_TIMERS
  std::cout << "  Timer build option is enabled. Current timer level is "
...
{ std::cout<<"AV entering QMCMain::execute"<<std::endl;
  std::flush(std::cout);
  Timer t0;
  ...
  //std::cout<<"AV in QMCMain::execute, before validateXML"<<std::endl;
  //std::flush(std::cout);
  //validate the input file
  bool success = validateXML();
  //std::cout<<"AV in QMCMain::execute, after validateXML"<<std::endl;
  //std::flush(std::cout);
  if (!success)
  ...
  ptclPool->reset();  //ParticleSetPool::reset ->ParticleSet::update ->SoaDistanceTableAA.h::evaluate -> ParticleBConds2DSoa.h::computeDistances (fill r distance table etc)
  infoSummary.flush();
  infoLog.flush();
  ...
  for (int qa = 0; qa < m_qmcaction.size(); qa++)
  {
    ...
    if (cname == "qmc" || cname == "optimize")
    {
      ...
      std::cout<<"AV in QMCMain::execute, before executeQMCSection (optimize)"<<std::endl<<std::endl;
    ...
    {
      std::cout<<std::endl<<std::endl;
      std::cout<<"AV in QMCMain::execute, before executeLoop (also optimize?)"<<std::endl<<std::endl;
...
{
  std::cout<<"AV entering QMCMain::executeLoop"<<std::endl;
  std::flush(std::cout);
  ...
  app_log() << "Loop execution max-iterations = " << niter << std::endl;

  //AV what is the iteration here??
  for (int iter = 0; iter < niter; iter++)
  {
    ...
    while (tcur != NULL)
    {
      ...
      if (cname == "qmc")
      {
        ...
        std::cout<<"AV in QMCMain::executeLoop, before executeQMCSection, iter: "<<iter<<" niter: "<<niter<<std::endl<<std::endl;
        bool success = executeQMCSection(tcur, true);
        std::cout<<"AV in QMCMain::executeLoop, after executeQMCSection, iter: "<<iter<<" niter: "<<niter<<std::endl<<std::endl;
        if (!success)
        {
  ...
  std::cout<<"AV exiting QMCMain::executeLoop"<<std::endl<<std::endl;
}
...
{
  std::cout<<"AV entering QMCMain::validateXML"<<std::endl;
  std::flush(std::cout);
  ...
  while (cur != NULL)
  {
    ...
    {
      std::cout<<"AV in QMCMain::validateXML, before put put particle pool"<<std::endl;
      std::flush(std::cout);
      xmlNodePtr AVtestnode=xmlCopyNode(cur, 1);
      std::cout<<"AV in QMCMain::validateXML, before put particle node after AVtestnode"<<std::endl;
      std::flush(std::cout);
      ptclPool->put(cur);
      std::cout<<"AV in QMCMain::validateXML, after put particle node"<<std::endl;
      std::flush(std::cout);
      xmlNodePtr AVtestnode2=xmlCopyNode(cur, 1);
      std::cout<<"AV in QMCMain::validateXML, after put particle node after AVtestnode2"<<std::endl;
      std::flush(std::cout);
    }
    else if (cname == "wavefunction")
    { std::cout<<"AV in QMCMain::validateXML, before put psiPool"<<std::endl;
      std::flush(std::cout);
      xmlNodePtr AVtestnode=xmlCopyNode(cur, 1);
      std::cout<<"AV in QMCMain::validateXML, before put psiPool ater AVtestnode"<<std::endl;
      std::flush(std::cout);
      psiPool->put(cur);
      std::cout<<"AV in QMCMain::validateXML, after put psiPool"<<std::endl;
    }
...
{
  std::cout<<"AV entering QMCMain::runQMC (before making pointer to qmc_driver)"<<std::endl;

  //constructor does not (!) call QMCDriverFactory::createQMCDriver, createQMCDriver is called below
  std::unique_ptr<QMCDriverInterface> qmc_driver;
  infoSummary.flush();
  infoLog.flush();
  std::cout<<"AV in QMCMain::runQMC, after making pointer to qmc_driver"<<std::endl;
  ...
  else
  {
    ...
    std::cout<<"AV in QMCMain::runQMC, before createQMCDriver"<<std::endl;

    //QMCDriverFactory::createQMCDriver, set PrimaryPsi, PrimaryH
    qmc_driver = driver_factory.createQMCDriver(cur, das, *qmcSystem, *ptclPool, *psiPool, *hamPool, myComm);
    infoSummary.flush();
    infoLog.flush();
    std::cout<<"AV in QMCMain::runQMC, after createQMCDriver"<<std::endl;
  ...
  if (qmc_driver)
  {
    ...
    {
      std::cout<<"AV in QMCMain::runQMC, if last_branch_engine_lecacy_driver, before resetRun"<<std::endl;
    ...
    //std::cout<<"AV in QMCMain::runQMC, before setStatus"<<std::endl;
    //QMCDriver::setStatus (set names, print which optimization chosen)
    qmc_driver->setStatus(myProject.CurrentMainRoot(), "", append_run);
    //std::cout<<"AV in QMCMain::runQMC, after setStatus"<<std::endl;
    // PD:
    ...
    std::cout<<"AV in QMCMain::runQMC, before putWalkers"<<std::endl;
    
    //QMCDriver::putWalkers, set walkers if walkerset not empty    
    //(empty when doing restart, then putWalkers does nothing)
    qmc_driver->putWalkers(m_walkerset_in);
    infoSummary.flush();
    infoLog.flush();
    std::cout<<"AV in QMCMain::runQMC, after putWalkers"<<std::endl;
#if !defined(REMOVE_TRACEMANAGER)
    qmc_driver->putTraces(traces_xml);
#endif

    std::cout<<"AV in QMCMain::runQMC, before process"<<std::endl;
    //QMCDriver::process  Initialize basic data to execute run function: Distance tables, resize deltaR and drift with the number of particles, assign cur to qmcNode, process input file, putQMCInfo: <parameter/> s for generic QMC, put : extra data by derived classes (calls put in QMCFixedSampleLinearOptimize, AttributeSet, ParameterSet, OhmmsParameter, VMC, SimpleFixedNodeBranch), initialize branchEngine to accumulate energies, initialize Estimator, initialize Walkers
    qmc_driver->process(cur);
    std::cout<<"AV in QMCMain::runQMC, after process"<<std::endl;
    ...
    std::cout<<"AV in QMCMain::runQMC, before qmc_driver->run()"<<std::endl<<std::endl;
    
    // QMCFixedSampleLinearOptimize::run->QMCLinearOptimize::start->QMCLinearOptimize::GenerateSamples->VMC::run (maybe also others) (VMC derived class of QMCDriver)
    //samples + observables (energy, variance) evaluated on observables created in call QMCLinearOptimize::start (and then calling eventually VMC::run). Optimization step (determining and setting new parameter values, difference of cost function (energy / variance) with new parameters calculated) done in QMCFixedSampleLinearOptimize::run, after back from call QMCLinearOptimize::start()
    qmc_driver->run();

    std::cout<<"AV in QMCMain::runQMC, after qmc_driver->run()"<<std::endl<<std::endl;
    std::cout << "  QMC Execution time = " << std::setprecision(4) << qmcTimer.elapsed() << " secs" << std::endl;
    ...
    if (reuse){
      ...
      std::cout<<"AV exiting QMCMain::runQMC"<<std::endl<<std::endl;}
    return true;
  }
  else
  {std::cout<<"AV exiting QMCMain::runQMC"<<std::endl<<std::endl;
