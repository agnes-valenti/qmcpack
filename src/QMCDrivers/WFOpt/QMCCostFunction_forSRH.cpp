                                                                           ...
                                                                           Matrix<Return_rt>& S)
{
  ...
  Matrix<RealType> S2(getNumParams(),getNumParams());
  Matrix<RealType> S3;(getNumParams(),getNumParams());
    ...
    for (int iw = 0; iw < nw; iw++)
    {
      ...
      for (int pm = 0; pm < getNumParams(); pm++)
      {
        ...
        for (int pm2 = 0; pm2 < getNumParams(); pm2++){
          ...
          S2(pm,pm2)+=HDsaved[pm]*Dsaved[pm2]*weight;
          S3(pm,pm2)+=Dsaved[pm]*Dsaved[pm2]*eloc_new*weight
        }
...
#pragma omp parallel for
  for (int pm = 0; pm < getNumParams(); pm++)
  {
    ...
    fk(pm) = -2.0*(ElocD_avg[pm]-Eloc_avg[pm]*D_avg[pm]);
    //fk(pm) = -(HD_avg[pm]);

    //std::cout<<"fk: "<<fk(pm)<<" HD_avg: "<<HD_avg[pm]<< "ElocD_avg: "<<ElocD_avg[pm]<<std::endl;
    for (int pm2 = 0; pm2 < getNumParams(); pm2++){
      S(pm,pm2)=S1(pm,pm2)-D_avg[pm]*D_avg[pm2];
      Sh(pm,pm2)=S2(pm,pm2)+S2(pm2,pm)-HD_avg[pm]*D_avg[pm2]-HD_avg[pm2]*D_avg[pm];
      G(pm,pm2)=2*(S3(pm,pm2)+Eloc_avg[pm]*(2*D_avg[pm]*D_avg[pm2]-S1(pm,pm2))-D_avg[pm]*ElocD_avg[pm2]);
    }
