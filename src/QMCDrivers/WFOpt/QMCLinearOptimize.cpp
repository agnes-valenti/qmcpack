...
{  //called (probably) from QMCFixedSampleLinearOptimize::run
  std::cout<<"AV entering QMCLinearOptimize::start "<<std::endl;
  {
    //generate samples
    ScopedTimer local(generate_samples_timer_);

    //QMCLinearOptimize::generateSamples -> VMC::run, run optimization step (MC chain etc.)
    generateSamples();
    //store active number of walkers
    NumOfVMCWalkers = W.getActiveWalkers();
  }

  std::cout << "<opt stage=\"setup\">" << std::endl;
  std::cout << "  <log>" << std::endl;
  //reset the rootname
  optTarget->setRootName(RootName);
  optTarget->setWaveFunctionNode(wfNode);
  std::cout << "  (AV) Reading configurations from h5FileRoot " << h5FileRoot << std::endl;
  {
    ...
    //QMCCostFunction::checkConfigurations, compute d_alpha Psi/Psi, d_alpha HPsi/Psi as well as local energy again from samples (separated for optimizable/ non-optimizable part)
    ...
    std::cout<<"AV in QMCLinearOptimize, sigma2_vmc: "<<sigma2_vmc<<" sigma2_check: "<<sigma2_check<<std::endl;
    if (sigma2_check > 2.0 * sigma2_vmc || sigma2_check < 0.5 * sigma2_vmc){  //why different than before?? Check, compared to previously calculated!
      //throw std::runtime_error(
          std::cout<<"Safeguard failure: checkConfigurations variance out of [0.5, 2.0] * reference! Please report this bug.\n";
    }
    std::cout << " 1 Execution time = " << std::setprecision(4) << t2.elapsed() << std::endl;
  }
  std::cout << "  </log>" << std::endl;
  std::cout << "</opt>" << std::endl;
  std::cout << "<opt stage=\"main\" walkers=\"" << optTarget->getNumSamples() << "\">" << std::endl;
  std::cout << "  <log>" << std::endl;
  t1.restart();  
  std::cout<<"AV exiting QMCLinearOptimize::start"<<std::endl<<std::endl;
}
                                     ...
                                     std::string MinMethod)
{
  ...
  app_log() << "     Reading configurations from h5FileRoot " << h5FileRoot << std::endl;
  {
    ...
    app_log() << " (AV engine start) Execution time = " << std::setprecision(4) << t2.elapsed() << std::endl;
  }
  app_log() << "  </log>" << std::endl;
...
{  
  std::cout<<"AV entering QMCLinearOptimize::generateSamples"<<std::endl;
  app_log() << "<optimization-report>" << std::endl;
  ...
  branchEngine->reset();
  std::cout<<"AV in QMCLinearOptimize::generateSamples, before run"<<std::endl;

  //VMC::run, run VMC optimization step
  vmcEngine->run();

  std::cout<<"AV in QMCLinearOptimize::generateSamples, after run"<<std::endl;
  app_log() << " 2 (AV) Execution time = " << std::setprecision(4) << t1.elapsed() << std::endl;
  app_log() << "</vmc>" << std::endl;
  h5FileRoot = RootName;
  std::cout<<"AV exiting QMCLinearOptimize::generateSamples"<<std::endl;
}
