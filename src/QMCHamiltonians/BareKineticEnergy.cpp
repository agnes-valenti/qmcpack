...
BareKineticEnergy::BareKineticEnergy(ParticleSet& p) : Ps(p)
{
  ...
  double mstar_=0.457*me_*qe_/(hbar_*hbar_*1e3)*a_*a_; //1.0; //0.457*me_*qe_/(hbar_*hbar_*1e3)*a_*a_; //AV: esquared in Coulomb interaction is then in units of mstar=0.457*me_*qe_/(hbar_*hbar_*1e3)*a_*a_; 
  
  std::cout<<std::endl<<std::endl;
  std::cout<<"AV mstar_: "<<mstar_<<" 1/mstar_: "<<1.0/mstar_<<std::endl<<std::endl;

  M           = tspecies(massind, 0)*mstar_;
  OneOver2M   = 0.5 / M;
  for (int i = 0; i < tspecies.size(); ++i)
  {
    SameMass &= (std::abs(tspecies(massind, i)*mstar_ - M) < 1e-6);
    MinusOver2M[i] = -1.0 / (2.0 * tspecies(massind, i)*mstar_);
  }
}
...
{
//std::cout<<"AV entering BareKineticEnergy::evaluate"<<std::endl;
#if !defined(REMOVE_TRACEMANAGER)
  if (streaming_particles_)
  {
    value_ = evaluate_sp(P);
      ...
      if (SameMass)
  {
   //std::cout<<"AV in BareKineticEnergy::evaluate, same mass"<<std::endl;
   //(for formula, see casino manual calculation of kinetic energy)
   //Achtung sum negativ! Vorzeichen umgekehrt zur eigentlichen kinetischen Energie
#ifdef QMC_COMPLEX
    value_ = std::real(CplxDot(P.G, P.G) + CplxSum(P.L));
    //std::cout<<"AV in BareKineticEnergy::evaluate, f squared: "<<CplxDot(P.G, P.G)<<" laplacian sum: "<<CplxSum(P.L)<<std::endl;

    value_ *= -OneOver2M;
#else
    //std::cout<<"AV in BareKineticEnergy::evaluate, P.G size: "<<P.G.size()<<std::endl;
    value_ = Dot(P.G, P.G) + Sum(P.L);  //G: gradient, L: laplacian
    value_ *= -OneOver2M;
#endif
  }
  else
  {
    //std::cout<<"AV in BareKineticEnergy::evaluate, not same mass"<<std::endl;
    value_ = 0.0;
    for (int i = 0; i < MinusOver2M.size(); ++i)
    {
      Return_t x = 0.0;
      for (int j = P.first(i); j < P.last(i); ++j)
        x += laplacian(P.G[j], P.L[j]);   //in 2D bzw. twice in 1D, different mass
      value_ += x * MinusOver2M[i];
    }
  }
  //std::cout<<"AV exiting BareKineticEnergy::evaluate"<<std::endl<<std::endl;
  //value_=-value_;//AV change: minus sign. Important to change value_, 
                 // not only return statement: member variable of OperatorBase
  return value_;  
}
