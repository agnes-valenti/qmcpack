 ...
 *
 * AV: 2D implementation from Casino manual (19.4.2, 2D Ewald interaction, eqs. (147) and (149)).
 *
 */
...
{
  DIM = 2
};  //AV changed 3 to 2 -> careful, adapt rest of code also!!
...
// AV change: 2D from Casino Manual, formula (149)
...
private:
  ...
  // 1/(2*sqrt(gamma)) -> with other kappa, 1/(2*kappa)
  const real_t kconst;
...
public:
  ...
  real_t operator()(const IntVec& i) const
  {
    RealVec Kv = dot(i, b);
    //real_t K2  = dot(Kv, Kv);
    real_t K2  = std::sqrt(dot(Kv, Kv));
    //real_t km  = kfactor * std::exp(kconst * K2) / K2;
    real_t km  = kfactor*std::erfc(kconst*K2)/K2;
    return km;
  }
};
...
// Same for 3D and 2D, only changed dim (Casino manual, formula (147))
class RspaceEwaldTerm
{
private:
  /// The inter-particle separation vector
  const RealVec r;
  /// The real-space cell axes (rescale -> R_s) (?)
  const RealMat a;

  /// The constant 1/(\sqrt{2}\kappa) in Drummond 2008 formula 6
  // (same constant, =sqrt(gamma))
  const real_t rconst;
...
public:
  ...
  {
    RealVec Rv = dot(i, a);
    for (int_t d : {0, 1})   //change dim here!
      Rv[d] -= r[d];
    real_t R  = std::sqrt(dot(Rv, Rv));
    real_t rm = std::erfc(rconst * R) / R;
    return rm;
...
// AV: 2D, changed to term in formula (147) from Casino manual
class KspaceEwaldTerm
{
private:
  /// The inter-particle separation vector
  const RealVec r;
  /// The k-space cell axes (rescale -> G_s) (?)
  const RealMat b;

  /// The constant -\kappa^2/2 in Drummond 2008 formula 6
  //1/(2sqrt(gamma))=kappa/sqrt(2)
  const real_t kconst;
...
public:
  ...
  real_t operator()(const IntVec& i) const
  {
    ...
    real_t K2  = std::sqrt(dot(Kv, Kv));
    real_t Kr  = dot(Kv, r);
    real_t km  = kfactor * std::erfc(kconst * K2) * std::cos(Kr) / K2;
    return km;
  }
};
...
real_t gridSum(T& function, bool zero = true, real_t tol = 1e-11)
{
  ...
  while (std::abs(dva) > tol)
  {
    ...
    im += 1;
    for (int_t i : {-im, im})
      for (int_t j = -jm; j < jm + 1; ++j)  //AV: removed sum over z for 2D
        {
          iv[0]    = i;
          iv[1]    = j;
          //iv[2]    = k;
    ...
    // Sum over new surface planes perpendicular to the y direction. (replace with lines instead of surfaces for 2D)
    jm += 1;
    for (int_t j : {-jm, jm})
    ...
    // Sum over new surface planes perpendicular to the z direction. (AV: part removed for 2D)
...
{
  real_t radius = std::pow(3. * volume / (4 * M_PI), 1. / 3);  //2D?
  //real_t radius=std::pow(volume / (M_PI), 1. / 2);
  return std::sqrt(M_PI) / radius;
}

real_t getGamma(real_t area)
{
  return std::pow(2.4/std::sqrt(area),2);
}
...
real_t madelungSum(const RealMat& a, real_t tol = 1e-10)
{
  ...
  real_t gamma_=getGamma(volume);

  // Set constants for real-/k-space Madelung functors
  real_t rconst  = std::sqrt(gamma_);
  real_t kconst  = 1. / (2 * std::sqrt(gamma_));
  real_t kfactor = 2 * M_PI / volume;
  ...
  real_t cval = -2*std::sqrt(M_PI) / (rconst * volume) - 2 * rconst / std::sqrt(M_PI);
...
real_t ewaldSum(const RealVec& r, const RealMat& a, real_t tol = 1e-10)
{
  ...
  // Compute the constant term
  real_t cval = -2 * std::sqrt(M_PI) / (rconst * volume);
