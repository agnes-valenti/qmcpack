...
{
  int nc(0), nc2(-2);
  std::cout<<"AV 1nc: "<<nc<<std::endl;
  ValueType bosonic_eps(-999999);
  ValueType rntype(0);
  PosType twist(0.0);
  OhmmsAttributeSet aAttrib;
  aAttrib.add(nc, "shell");
  std::cout<<"AV 2nc: "<<nc<<std::endl;
  aAttrib.add(nc2, "shell2");
  std::cout<<"AV 3nc: "<<nc<<std::endl;
  aAttrib.add(bosonic_eps, "eps");
  aAttrib.add(rntype, "primary");
  aAttrib.add(twist, "twist");
  std::cout<<"AV 4nc: "<<nc<<std::endl;
  aAttrib.put(cur);
  std::cout<<"AV 4.5nc: "<<nc<<std::endl;
  if (nc2 == -2)
    nc2 = nc;
  std::cout<<"AV 5nc: "<<nc<<std::endl;
  ...
  std::cout<<"AV nat: "<<nat<<std::endl;
  std::cout<<"AV nc: "<<nc<<std::endl;
  if (1==1)   //AV, (change back to (nc == 0) )
    nc = nc2 = egGrid.getShellIndex(nat / 2);
  std::cout<<"AV nc: "<<nc<<std::endl;
  int nup = egGrid.getNumberOfKpoints(nc);
  std::cout<<"AV Number kpoints nup: "<<nup<<std::endl;
  int ndn(0);
  if (nc2 > -1)
  ...
  std::cout<<"AV Number kpoints ndn: "<<ndn<<std::endl;
  if (nat != (nup + ndn))
  {
    app_error() << "  The number of particles " << nup << "/" << ndn << " does not match to the shell. (real orbitals)" << std::endl;
...
std::unique_ptr<SPOSet> ElectronGasSPOBuilder::createSPOSetFromXML(xmlNodePtr cur, int particletype=0)
{
  int nc = 0;
  int ns = 0;
  PosType twist(0.0);
