...
struct RealEGOSet : public SPOSet
{
  ...
  {
    //std::cout<<"AV ElectronGasOrbitalBuilder evaluateValue"<<std::endl;
    const PosType& r = P.activeR(iat);
    //std::cout<<"AV in ElectronGasOrbitalBuilder evaluateValue, PosType& r=P.activeR(iat): "<<r<<std::endl;
    ...
    //std::cout<<"AV exiting ElectronGasOrbitalBuilder evaluateValue, psi (coskr, sinkr for all k in kgrid, pos r of active particle) filled"<<std::endl<<std::endl;
  }
                          ...
                          ValueVector_t& d2psi) override
  {
    ...
    {
      int j2 = j1 + 1;
      qmcplusplus::sincos(dot(K[ik], r), &sinkr, &coskr);  //make function 2D!!!
      psi[j1]   = coskr;
      psi[j2]   = sinkr;
      dpsi[j1]  = -sinkr * K[ik];  //AV: vector: put here different masses for directions (function is called separately for different spin species, can put it here already! Give valley index to function with default value. 2D: K 2D and kr 2D dot product
      dpsi[j2]  = coskr * K[ik];
      d2psi[j1] = mK2[ik] * coskr;  //AV: also here: mK2 (=-k^2) 2D + different masses in different directions!!
