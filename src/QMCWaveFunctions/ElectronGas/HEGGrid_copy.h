...
struct HEGGrid
{
  ...
  for (int i=0; i<kgridmax; i++){
    for (int j=0; j<kgridmax; j++){
      Kx[i][j]=-Kmax+i*deltak_linspace;
      Ky[i][j]=-Kmax+j*deltak_linspace;
  ...
  //deltak=AVkmax/halfnumberpointsm1;  //AV continue change here!!!!!!!!!------------------------------------------------------------
  //Kx_values=np.loadtxt("~/qmcpack-3.12.0/examples/electrongas/Kx.txt")
  //Kx_values=np.loadtxt("~/qmcpack-3.12.0/examples/electrongas/Kx.txt")
  
  //std::vector<std::vector<double> > Kx(kgridmax,std::vector<double>(kgridmax));
  //std::vector<std::vector<double> > Ky(kgridmax,std::vector<double>(kgridmax));

  //std::string filename_kx="Kx.txt";
  //std::ifstream finx(filename_kx.c_str());
  //if(!finx.good()){
  //  std::cerr<<"# Error : Cannot load from file "<<filename_kx<<" : file not found."<<std::endl;
  //  std::abort();
  //}

  //std::string filename_ky="Ky.txt";
  //std::ifstream finy(filename_ky.c_str());
  //if(!finy.good()){
  //  std::cerr<<"# Error : Cannot load from file "<<filename_ky<<" : file not found."<<std::endl;
  //  std::abort();
  //}

  //for(int i=0; i<kgridmax; i++){  //change!!
  //  for (int j=0; j<kgridmax; j++){
  //    finx>>Kx[i][j];
  //    finy>>Ky[i][j];}
  //    }
      ...
      for (int i1 = 0 ; i1 < kgridmax ; ++i1)
        {
          if (Pmatrix_[k_it][nc]>0.5){
            ...
            std::cout<<"AV in HEGGrid::create_kpoint, k cart: "<<kp->k<<" k: "<<k<<std::endl;
            kp->k2 = dot(k,k);
