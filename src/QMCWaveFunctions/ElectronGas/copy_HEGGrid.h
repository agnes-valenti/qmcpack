...
struct HEGGrid
{
  ...
  {
    std::cout<<"AV in HEGGrid::getShellIndex, Number of k-points: "<<nkpt<<std::endl;
    auto loc = std::upper_bound(n_within_shell.begin(), n_within_shell.end(), nkpt);
    if (loc < n_within_shell.end()){
      std::cout<<"AV in HEGGrid::getShellIndex, shell index: "<<loc<< " "<<loc - n_within_shell.begin() - 1<<" "<<n_within_shell[0]<<" "<<n_within_shell[1]<<" "<<n_within_shell[loc - n_within_shell.begin() - 1]<<std::endl;
  ...
  {  //read file here!!
    ...
    while (ikpt < nkpts && rs_it != rs_end)
    {
      ...
      std::cout<<"AV in HEGGrid::createGrid, ikpt: "<<ikpt<<" *ns_it: "<< *ns_it<<std::endl;
    ...
    app_log() << "List of kpoints (half-sphere), number kpoints: "<<kpt.size() << std::endl;
