...
DiracDeterminantWithBackflow::PsiValueType DiracDeterminantWithBackflow::ratio(ParticleSet& P, int iat)
{
  ...
  std::cout<<std::endl;
  std::cout<<"iat "<<iat<<" logvalue: "<<log_value_<<" new log: "<<NewLog<<std::endl;
  std::cout<<std::endl;
    ...
    ParticleSet::ParticleLaplacian_t& L)
{
  ...
  for (int i = 0; i < num; i++)
  {
    ...
    for (int j = 0; j < NumPtcls; j++)
    {
      ...
      std::cout<<"Amat(i,j): "<<" i: "<<i<<"j: "<<FirstIndex+j<<" Amat: "<<BFTrans_.Amat(i, FirstIndex + j)<<std::endl;
      //temp2 += rcdot(BFTrans_.Bmat_full(i,FirstIndex+j),Fmat(j,j));
    }
    std::cout<<"myG[i] vor temp: "<<myG[i]<<std::endl;
    myG[i] += temp;
    std::cout<<"myG[i]: "<<myG[i]<<std::endl;
    myL[i] += temp2;
  }
  ...
  for (int i = 0; i < num; i++)
  {
    ...
    G[i] += myG[i];
    //AV: L and G tested (for i = 0), match with python numerical deriv! 
  }
  return log_value_;
}
                                                       ...
                                                       std::vector<ValueType>& dhpsioverpsi)
{
  ...
  //for (int pa=0; pa<BFTrans_.numParams; ++pa)
  {
    ...
    for (int i = 0; i < num; i++)
    {
      ...
      Gtemp[i] += temp;
      std::cout<<"param: "<<pa<<" i: "<<i<<" Grad deriv, Gtemp[i]: "<<Gtemp[i]<<std::endl;
    }
    for (int j = 0; j < NumPtcls; j++)
    {
      GradType B_j;
      ...
      std::cout<<"j "<<j<<" param: " <<pa<<" Fmat(j,j) "<<Fmat(j,j)<<" Cmat("<<pa<<", "<<FirstIndex+j<<" ): "<<BFTrans_.Cmat(pa,FirstIndex+j)<<std::endl;
    ...
    std::cout<<"kk: "<<kk<<" dLa: "<<dLa<<std::endl;
...
void DiracDeterminantWithBackflow::evaluateDerivatives(ParticleSet& P,
                                                       ...
                                                       Matrix<RealType>& dL)
{ 
  std::cout<<"AV in DiracDeterminantWithBackflow::evaluateDerivatives, needs to be implemented for anisotropic" <<std::endl;
  std::flush(std::cout);
  abort();
  /*  Note:
   *    Since evaluateDerivatives seems to always be called after
