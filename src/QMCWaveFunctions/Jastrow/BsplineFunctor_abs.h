...
struct BsplineFunctor : public OptimizableFunctorBase
{
  ...
  void resize(int n)
  {
    ...
    int numCoefs = (NumParamsu + 4+3*Nv); //4;  //AV changed for without cusp!
  ...
  void reset() override
  {
    ...
    //std::cout<<"etaVar: "<<etaVar<<std::endl;
    const int numCoefs = NumParamsu+4+3*Nv;  // + 4;  //AV changed for without cusp!
...
void evaluateVGL2(const int iat,
  ...
  { 
    ...
    int deriv_shift_beta=nparamsu+nv+nv;

    if (tauvalue>1.5){
      shift_index_alpha=nparamsu+4+nv+nv;
      shift_index_beta=nparamsu+4+nv;
      deriv_shift_alpha=nparamsu+nv+nv;
      deriv_shift_beta=nparamsu+nv;
    }
    ...
    */
    ...
    if (optimizeCAlphaBeta){
      for (int i_nv=0; i_nv<nv; i_nv++){
        ...
        derivs[deriv_shift_alpha+i_nv][0]=std::pow(Lx,-(i_nv+1.0))*(i_nv+1)/2.0*coefs[shift_index_c+i_nv]*std::pow(r_an,((i_nv+1)/2.0-1.0))*fx*fx*coefs[shift_index_alpha+i_nv]/std::sqrt(coefs[shift_index_alpha+i_nv]* coefs[shift_index_alpha+i_nv]);
        derivs[deriv_shift_beta+i_nv][0]=std::pow(Lx,-(i_nv+1.0))*(i_nv+1)/2.0*coefs[shift_index_c+i_nv]*std::pow(r_an,((i_nv+1)/2.0-1.0))*fy*fy*coefs[shift_index_beta+i_nv]/std::sqrt(coefs[shift_index_beta+i_nv]* coefs[shift_index_beta+i_nv]);
...
#pragma omp simd reduction(+ : d)
  for (int jat = 0; jat < iCount; jat++)
  {
    ...
    for (int i_nv=0; i_nv<nv; i_nv++){
      double r_an=std::sqrt(coefs[shift_index_alpha+i_nv]*coefs[shift_index_alpha+i_nv])*fx*fx+std::sqrt(coefs[shift_index_beta+i_nv]*coefs[shift_index_beta+i_nv])*fy*fy;
      if (std::abs(r_an)<1e-15){
        r_an+=1e-15;
      }
      vsum+=std::pow(Lx,-(i_nv+1.0))*coefs[shift_index_c+i_nv]*std::pow(r_an,((i_nv+1)/2.0));
  ...
  for (int j = 0; j < iCount; j++)
  {
    ...
    */
    ...
    for (int i_nv=0; i_nv<nv; i_nv++){
      ...
      vsum+=std::pow(Lx,-(i_nv+1.0))*coefs[shift_index_c+i_nv]*std::pow(r_an,((i_nv+1)/2.0));
      vxderiv+=std::pow(Lx,-(i_nv+1.0))*coefs[shift_index_c+i_nv]*n/2.0*std::pow(r_an,(n/2.0-1))*(2.0*std::sqrt(coefs[shift_index_alpha+i_nv]*coefs[shift_index_alpha+i_nv])*fx*dfxdx);
      
      vyderiv+=std::pow(Lx,-(i_nv+1.0))*coefs[shift_index_c+i_nv]*n/2.0*std::pow(r_an,(n/2.0-1))*(2.0*std::sqrt(coefs[shift_index_beta+i_nv]*coefs[shift_index_beta+i_nv])*fy*dfydy);
      
      vx2deriv+=std::pow(Lx,-(i_nv+1.0))*coefs[shift_index_c+i_nv]*n/2.0*((n/2.0-1)*std::pow(r_an,(n/2.0-2))*(2.0*std::sqrt(coefs[shift_index_alpha+i_nv]*coefs[shift_index_alpha+i_nv])*fx*dfxdx)*(2.0*std::sqrt(coefs[shift_index_alpha+i_nv]*coefs[shift_index_alpha+i_nv])*fx*dfxdx)
        + std::pow(r_an,(n/2.0-1))*(2.0*std::sqrt(coefs[shift_index_alpha+i_nv]* coefs[shift_index_alpha+i_nv])*(dfxdx*dfxdx+fx*d2fxdx2))); 

      vy2deriv+=std::pow(Lx,-(i_nv+1.0))*coefs[shift_index_c+i_nv]*n/2.0*((n/2.0-1)*std::pow(r_an,(n/2.0-2))*(2.0*std::sqrt(coefs[shift_index_beta+i_nv]* coefs[shift_index_beta+i_nv])*fy*dfydy)*(2.0*std::sqrt(coefs[shift_index_beta+i_nv]*coefs[shift_index_beta+i_nv])*fy*dfydy)
        + std::pow(r_an,(n/2.0-1))*(2.0*std::sqrt(coefs[shift_index_beta+i_nv]*coefs[shift_index_beta+i_nv])* (dfydy*dfydy+fy*d2fydy2))); 
