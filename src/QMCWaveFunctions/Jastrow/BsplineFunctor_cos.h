...
struct BsplineFunctor : public OptimizableFunctorBase
{
  ...
  real_type DeltaR, DeltaRInv, DeltaRu, DeltaRInvu;
  real_type CuspValue;
  real_type Y, dY, d2Y;
  ...
  bool optimizeU, optimizeCAlphaBeta;
  ...
  void resize(int n)
  {
    ...
    int numCoefs = (NumParamsu + 4+2*Nv); //4;  //AV changed for without cusp!
    
    int numKnotsu = NumParamsu+4 - 2 ;  //- 2;  //AV changed for without cusp!
  ...
  void reset() override
  {
    ...
    //std::cout<<"etaVar: "<<etaVar<<std::endl;
    const int numCoefs = NumParamsu+4+2*Nv;  // + 4;  //AV changed for without cusp!
    ...
    */
    ...
    for (int i = 0; i < Nv; i++)
      coefs[NumParamsu+1+3+Nv+i] = Parameters[NumParamsu+Nv+i];

    //for (int i = 0; i < Nv; i++)
    //  coefs[NumParamsu+1+3+Nv+Nv+i] = Parameters[NumParamsu+Nv+Nv+i];

    
    
    cutoff_new=cutoff_radius;
...
void evaluateVGL2(const int iat,
  ...
  inline static real_type evaluate_impl(real_type r, real_type x, real_type y, const real_type* coefs,  int tauvalue, real_type Lx, int nparamsu, int nv) //const
  {
    std::cout<<"AV in BsplineFunctor::evaluate1, not implemented for anisotropy"<<std::endl;
    std::flush(std::cout);
    abort();
  ...
  inline real_type evaluate(real_type r, real_type x, real_type y, int numpart, int tauvalue) const
  {
    ...
    abort();
    real_type u(0);
    u = evaluate_impl(r,x,y, spline_coefs_->data(), tauvalue, length/2.0, NumParamsu, Nv);
  ...
  { 
    ...
    int shift_index_c=nparamsu+4;
    int shift_index_alpha=nparamsu+4+nv;
    int shift_index_beta=nparamsu+4+nv;

    int deriv_shift_c=nparamsu;
    int deriv_shift_alpha=nparamsu+nv;
    int deriv_shift_beta=nparamsu+nv;

    double shiftalpha=0;
    double shiftbeta=-M_PI/2.0;
    ...
    for (int i_nv=0; i_nv<nv; i_nv++){
      double n=i_nv+1.0;
      double factor_alpha=std::cos(coefs[shift_index_alpha+i_nv]+shiftalpha)*std::cos(coefs[shift_index_alpha+i_nv]+shiftalpha);
      double factor_beta=std::cos(coefs[shift_index_beta+i_nv]+shiftbeta)*std::cos(coefs[shift_index_beta+i_nv]+shiftbeta);
      double r_an=factor_alpha*fx*fx+factor_beta*fy*fy;
      if (std::abs(r_an)<1e-15){
        r_an+=1e-15;
      }
      vsum+=coefs[shift_index_c+i_nv]*std::pow(Lxfactor*r_an,((i_nv+1)/2.0));
      
      double coef_c_new=coefs[shift_index_c+i_nv];
      if (i_nv==ideriv && tauvalue!=0){
        coef_c_new=coefs[shift_index_c+i_nv]+eps;
      }
      double factor_alpha_new=std::cos(coefs[shift_index_alpha+i_nv]+shiftalpha)*std::cos(coefs[shift_index_alpha+i_nv]+shiftalpha);
      double factor_beta_new=std::cos(coefs[shift_index_beta+i_nv]+shiftbeta)*std::cos(coefs[shift_index_beta+i_nv]+shiftbeta);
      
      //double coef_alpha_new=coefs[shift_index_alpha+i_nv];
      //if (i_nv==ideriv && tauvalue!=0){
      //  coef_alpha_new=coefs[shift_index_alpha+i_nv]+eps;
      //  factor_alpha_new=std::cos(coef_alpha_new+shiftalpha)*std::cos(coef_alpha_new+shiftalpha);
      //  factor_beta_new=std::cos(coef_alpha_new+shiftbeta)*std::cos(coef_alpha_new+shiftbeta);
      //}

      vsumnew+=coef_c_new*std::pow(Lxfactor*(factor_alpha_new*fxnew*fxnew+factor_beta_new*fynew*fynew),((i_nv+1)/2.0));
    ...
    */
    ...
    if (optimizeCAlphaBeta){
      for (int i_nv=0; i_nv<nv; i_nv++){
        double factor_alpha=std::cos(coefs[shift_index_alpha+i_nv]+shiftalpha)*std::cos(coefs[shift_index_alpha+i_nv]+shiftalpha);
        double factor_beta=std::cos(coefs[shift_index_beta+i_nv]+shiftbeta)*std::cos(coefs[shift_index_beta+i_nv]+shiftbeta);
        double deriv_factor_alpha=-2.0*std::cos(coefs[shift_index_alpha+i_nv]+shiftalpha)*std::sin(coefs[shift_index_alpha+i_nv]+shiftalpha);
        double deriv_factor_beta=-2.0*std::cos(coefs[shift_index_beta+i_nv]+shiftbeta)*std::sin(coefs[shift_index_beta+i_nv]+shiftbeta);
        //std::cout<<" deriv factor alpha: "<<deriv_factor_alpha<<std::endl;
        //std::cout<<" deriv factor beta: "<<deriv_factor_beta<<std::endl;



        real_type r_an=factor_alpha*fx*fx+ factor_beta*fy*fy;
        if (std::abs(r_an)<1e-15){
          r_an+=1e-15;
        }
        //real_type vderiv=std::pow(r_an,((i_nv+1)/2.0));
        derivs[deriv_shift_c+i_nv][0]=std::pow(Lxfactor*r_an,((i_nv+1)/2.0));
        derivs[deriv_shift_alpha+i_nv][0]=Lxfactor*(i_nv+1)/2.0*coefs[shift_index_c+i_nv]*std::pow(Lxfactor*r_an,((i_nv+1)/2.0-1.0))*fx*fx*deriv_factor_alpha;
        
        derivs[deriv_shift_beta+i_nv][0]=derivs[deriv_shift_beta+i_nv][0]+ Lxfactor*(i_nv+1)/2.0*coefs[shift_index_c+i_nv]*std::pow(Lxfactor*r_an,((i_nv+1)/2.0-1.0))*fy*fy*deriv_factor_beta;

         //if (i_nv==ideriv && tauvalue!=0){
          //double coef_alpha_new=coefs[shift_index_alpha+i_nv]+eps;
          //double factor_alpha_new=std::cos(coef_alpha_new+shiftalpha)*std::cos(coef_alpha_new+shiftalpha);
        
          //double vsumtest=coefs[shift_index_c+i_nv]*std::pow(Lxfactor*r_an,((i_nv+1)/2.0));
          //double vsumnewtest=coefs[shift_index_c+i_nv]*std::pow(Lxfactor*(factor_alpha_new*fx*fx+factor_beta*fy*fy),((i_nv+1)/2.0));
          //std::cout<<"finite diff test: "<<(vsumnewtest-vsumtest)/eps<<std::endl;
          //double derivtest=Lxfactor*(i_nv+1)/2.0*coefs[shift_index_c+i_nv]*std::pow(Lxfactor*r_an,((i_nv+1)/2.0-1.0))*fx*fx*deriv_factor_alpha;
          //std::cout<<"derivtest: "<<derivtest<<std::endl;
  ...
  for (int j = 0; j < iCount; j++)
  {
    ...
    */
    ...
    for (int i_nv=0; i_nv<nv; i_nv++){
      ...
      vsum+=coefs[shift_index_c+i_nv]*std::pow(Lxfactor*r_an,((i_nv+1)/2.0));
      vxderiv+=Lxfactor*coefs[shift_index_c+i_nv]*n/2.0*std::pow(Lxfactor*r_an,(n/2.0-1))*(2.0*factor_alpha*fx*dfxdx);
      
      vyderiv+=Lxfactor*coefs[shift_index_c+i_nv]*n/2.0*std::pow(Lxfactor*r_an,(n/2.0-1))*(2.0*factor_beta*fy*dfydy);
      
      vx2deriv+=Lxfactor*coefs[shift_index_c+i_nv]*n/2.0*((n/2.0-1)*Lxfactor*std::pow(Lxfactor*r_an,(n/2.0-2))*(2.0*factor_alpha*fx*dfxdx)*(2.0*factor_alpha*fx*dfxdx)
        + std::pow(Lxfactor*r_an,(n/2.0-1))*(2.0*factor_alpha*(dfxdx*dfxdx+fx*d2fxdx2))); 

      vy2deriv+=Lxfactor*coefs[shift_index_c+i_nv]*n/2.0*((n/2.0-1)*Lxfactor*std::pow(Lxfactor*r_an,(n/2.0-2))*(2.0*factor_beta*fy*dfydy)*(2.0*factor_beta*fy*dfydy)
        + std::pow(Lxfactor*r_an,(n/2.0-1))*(2.0*factor_beta* (dfydy*dfydy+fy*d2fydy2))); 

      //------ AVTEST test v deriv ------------------
      //double r_annew=factor_alpha*fxnew*fxnew+factor_beta*fynew*fynew;
      //vsumnew+=coefs[shift_index_c+i_nv]*std::pow(Lxfactor*r_annew,((i_nv+1)/2.0));
      ...
      //double r_annew=coefs[shift_index_alpha+i_nv]*fxnew*fxnew+coefs[shift_index_beta+i_nv]*fynew*fynew;
      //vxderivnew+=Lxfactor*coefs[shift_index_c+i_nv]*n/2.0*std::pow(Lxfactor*r_annew,(n/2.0-1))*(2.0*factor_alpha*fxnew*dfxdxnew);
      //vyderivnew+=Lxfactor*coefs[shift_index_c+i_nv]*n/2.0*std::pow(Lxfactor*r_annew,(n/2.0-1))*(2.0*factor_beta*fynew*dfydynew);

      //vxderivnew+=std::pow(Lx,-(i_nv+1.0))*coefs[shift_index_c+i_nv]*n/2.0*std::pow(r_annew,(n/2.0-1))*(2.0*coefs[shift_index_alpha+i_nv]*fxnew*dfxdxnew);
      //vyderivnew+=std::pow(Lx,-(i_nv+1.0))*coefs[shift_index_c+i_nv]*n/2.0*std::pow(r_annew,(n/2.0-1))*(2.0*coefs[shift_index_beta+i_nv]*fynew*dfydynew);
      //double dv_=(vsumnew-vsum)/eps;
      //double dv_=(vyderivnew-vyderiv)/eps;
      //double dv_=(vxderivnew-vxderiv)/eps;

      //std::cout<<"i_nv: "<<i_nv<<" fx: "<<fx<<" coefs[c]: "<<coefs[shift_index_c+i_nv]<<std::endl;
      //std::cout<<"vyderiv: "<<vyderiv<<" finite diff: "<<dv_<<std::endl;
      //std::cout<<"vy2deriv: "<<vy2deriv<<" finite diff: "<<dv_<<std::endl;
      //std::cout<<"vx2deriv: "<<vx2deriv<<" finite diff: "<<dv_<<std::endl;

      //---------------------------------------------

    }

    laplArray_x[iScatter] = vx2deriv;
    laplArray_y[iScatter] = vy2deriv;


    gradArray_x[iScatter] = vxderiv;
    gradArray_y[iScatter] = vyderiv;

    valArray[iScatter] = vsum;
  }
}
