...
struct BsplineFunctor : public OptimizableFunctorBase
{
  ...
  double etaVar; //AV introduced for anisotropic
  double cutoff_new; //
  ...
  void setEtaVar(real_type etavar) override {etaVar=etavar;}
  ...
  void resize(int n)
  {
    ...
    NumParams    = n;
    NumParamsu=NumParams-NumParamsa2-NumParamsb2;
  ...
  void reset() override
  {
    ...
    */
    ...
    coefs[0]=coefs[2]-2*DeltaR*(CuspValue+(0.5*coefs[NumParamsu+3+0]-0.5*coefs[NumParamsu+3+2])*DeltaRInva2
             + (0.5*coefs[NumParamsu+3+NumParamsa2+3+0]-0.5*coefs[NumParamsu+3+NumParamsa2+3+2])*DeltaRInvb2);
...
void evaluateVGL2(const int iat,
  ...
  */
  ...
  inline bool evaluateDerivatives(real_type r, real_type x, real_type y, int& paramsu, int& paramsa2, int& paramsb2, real_type& etavar, std::vector<TinyVector<real_type, 3>>& derivs) override
  { 
    paramsu=NumParamsu;
    ...
    //cusp------
    //derivs[0]=0;
    derivs[2]=derivs[2]+SplineDerivs[0];
    derivs[0]=0;
    ...
    tp[2] = t;
    tp[3] = 1.0;
    int shift_index=NumParamsu+3;  //+3 or not??
    //i=i+shift_index;
    ...
    //cusp------
    //derivs[0]=0;
    derivs[0+NumParamsu]=derivs[0+NumParamsu]-SplineDerivs[0]*(DeltaRu*DeltaRInva2);

    derivs[2+NumParamsu]=derivs[2+NumParamsu]+SplineDerivs[0]*(DeltaRu*DeltaRInva2);
    //----------
    ...
    //cusp------
    //derivs[0]=0;
    derivs[0+NumParamsu+NumParamsa2]=derivs[0+NumParamsu+NumParamsa2]-SplineDerivs[0]*(DeltaRu*DeltaRInvb2);

    derivs[2+NumParamsu+NumParamsa2]=derivs[2+NumParamsu+NumParamsa2]+SplineDerivs[0]*(DeltaRu*DeltaRInvb2);
    //set derivs[0] to 0 later
    //----------
