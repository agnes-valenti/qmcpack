...
struct BsplineFunctor : public OptimizableFunctorBase
{
  ...
  real_type DeltaR, DeltaRInv, DeltaRu, DeltaRInvu,DeltaRa2, DeltaRInva2, DeltaRb2, DeltaRInvb2 ;
  real_type CuspValue;
  real_type Y, dY, d2Y;
  ...
  void resize(int n)
  {
    ...
    NumParams    = n;
    NumParamsu=NumParams-NumParamsa2-NumParamsb2-1;
    std::cout<<"NumParamsu: "<<NumParamsu<<  "NumParamsa2: "<<NumParamsa2<<" NumParamsb2: "<<NumParamsb2<<std::endl;

    int numCoefs = (NumParamsu + 3+NumParamsa2+3+NumParamsb2+3); //4;  //AV changed for without cusp!
  ...
  void reset() override
  {
    ...
    //std::cout<<"etaVar: "<<etaVar<<std::endl;
    const int numCoefs = NumParamsu+3+NumParamsa2+3+NumParamsb2+3;  // + 4;  //AV changed for without cusp!
    ...
    */
    ...
    for (int i = 0; i < NumParamsb2; i++)
      coefs[NumParamsu+3+NumParamsa2+3+i] = Parameters[NumParamsu+NumParamsa2+i];

    etaVar=Parameters[NumParamsu+NumParamsa2+NumParamsb2];
    if (etaVar>etaVarmax){
      etaVar=etaVarmax;
    ...
    if (etaVar<0){
      etaVar=1e-3;
      Parameters[NumParamsu+NumParamsa2+NumParamsb2]=etaVar;
    }
...
void evaluateVGL2(const int iat,
                   ...
                   int* restrict distIndices, real_type& etavar) const;
  ...
  T evaluateV2(const int iat,
              ...
              T* restrict distArrayCompressed,
              T* restrict displArrayCompressedXsquared,
              T* restrict displArrayCompressedYsquared) const;
  ...
  */
  ...
  inline real_type evaluate(real_type r, real_type xsquared, real_type ysquared, int numpart) const
  {
    ...
    if (r < cutoff_radius){
      if (numpart==0){ 
          u = evaluate_impl(std::sqrt(etaVar*xsquared+1.0/etaVar*ysquared), spline_coefs_->data(), DeltaRInva2, NumParamsu+3);}
      else if (numpart==1){
          u = evaluate_impl(std::sqrt(1.0/etaVar*xsquared+etaVar*ysquared), spline_coefs_->data(), DeltaRInvb2, NumParamsu+3+NumParamsa2+3);}
  ...
  inline real_type evaluate(real_type r, real_type xsquared, real_type ysquared, int numpart, real_type& dudr, real_type& d2udr2)
  {
    real_type u(0);
    dudr   = real_type(0);
    d2udr2 = real_type(0);
    ...
    if (r < cutoff_radius){
      if (numpart==0){ 
          u = evaluate_impl(std::sqrt(etaVar*xsquared+1.0/etaVar*ysquared), spline_coefs_->data(), DeltaRInva2, dudr, d2udr2, NumParamsu+3);}
      else if (numpart==1){
          u = evaluate_impl(std::sqrt(1.0/etaVar*xsquared+etaVar*ysquared), spline_coefs_->data(), DeltaRInvb2, dudr, d2udr2, NumParamsu+3+NumParamsa2+3);}
      }
  ...
  inline bool evaluateDerivatives(real_type r, real_type x, real_type y, int& paramsu, int& paramsa2, int& paramsb2, 
    ...
    double& dxdetaVar, double& dydetaVar, double& d2xdetaVar, double& d2ydetaVar) override
  { 
    ...
    tp[2] = t;
    tp[3] = 1.0;
    shift_index=NumParamsu+3;  //+3 or not??
    //i=i+shift_index;
    ...
    for (int n = imin; n < imax; ++n)
      derivs[n +NumParamsu] = SplineDerivs[n];  //derivs[n - 1] = SplineDerivs[n]; //AV change because of cusp condition
    
    //std::cout<<" a2 imin: "<<imin<< " imax: "<<imax<<std::endl;
    //std::cout<<" a2 imin: "<<imin+NumParamsu<< " imax: "<<imax+NumParamsu<<std::endl;

    //AV continue modifying here
    // AV put etaderiv a2 here -------------------------------------
    derivs[NumParamsu+NumParamsa2+NumParamsb2][0] = ((p0/2.0-p1+p2/2.0)*(x*x-y*y/(etaVar*etaVar))*t/(DeltaRa2*r0)
    +(-p0/2.0+p2/2.0)*(xp[1]-yp[1]/(etaVarp[2]))/(2*DeltaRa2p[1]*r0)
    +3*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*(xp[1]-yp[1]/(etaVarp[2]))*t*t/(2.0*DeltaRa2*r0)); 
    
    double xym=x*x-y*y/(etaVar*etaVar);
    dxdetaVar=-etaVar*(-p0/2.0+p2/2.0)*x*(xym)/(2*DeltaRa2*r0*r0*r0)
    +etaVar*(p0/2.0-p1+p2/2.0)*x*xym/(DeltaRa2*DeltaRa2*r0*r0)+(-p0/2.0+p2/2.0)*x/(DeltaRa2*r0)
    -etaVar*(p0/2.0-p1+p2/2.0)*x*xym*t/(DeltaRa2*r0*r0*r0)
    +3*etaVar*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*x*xym*t/(DeltaRa2*DeltaRa2*r0*r0)
    +2*(p0/2.0-p1+p2/2.0)*x*t/(DeltaRa2*r0)-3*etaVar*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*x*xym*t*t/(2*DeltaRa2*r0*r0*r0)
    +3*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*x*t*t/(DeltaRa2*r0);

    
    dydetaVar=-(-p0/2.0+p2/2.0)*y*xym/(2*DeltaRa2*etaVar*r0*r0*r0)
    +(p0/2.0-p1+p2/2.0)*y*xym/(DeltaRa2*DeltaRa2*etaVar*r0*r0)
    -(-p0/2.0+p2/2.0)*y/(DeltaRa2*etaVar*etaVar*r0)
    -(p0/2.0-p1+p2/2.0)*y*xym*t/(DeltaRa2*etaVar*r0*r0*r0)
    +3*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*y*xym*t/(DeltaRa2*DeltaRa2*etaVar*r0*r0)
    -2*(p0/2.0-p1+p2/2.0)*y*t/(DeltaRa2*etaVar*etaVar*r0)
    -3*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*y*xym*t*t/(2*DeltaRa2*etaVar*r0*r0*r0)
    -3*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*y*t*t/(DeltaRa2*etaVar*etaVar*r0);

    double p02=-p0/2.0+p2/2.0;
    double p012=p0/2.0-p1+p2/2.0;
    double p0123=-p0/6.0+p1/2.0-p2/2.0+p3/6.0;

    d2xdetaVar=3*etaVar*etaVar*(p02)*x*x*xym/(2*DeltaRa2*r0*r0*r0*r0*r0)
    -3*etaVar*etaVar*(p012)*x*x*xym/(DeltaRa2*DeltaRa2*r0*r0*r0*r0)
    -2*etaVar*(p02)*x*x/(DeltaRa2*r0*r0*r0) 
    - etaVar*(p02)*xym/(2*DeltaRa2*r0*r0*r0)
    + 3*etaVar*etaVar*(p0123)*x*x*xym/(DeltaRa2*DeltaRa2*DeltaRa2*r0*r0*r0)
    + 4*etaVar*(p012)*x*x/(DeltaRa2*DeltaRa2*r0*r0)
    +etaVar*(p012)*xym/(DeltaRa2*DeltaRa2*r0*r0)
    +(p02)/(DeltaRa2*r0)
    +3*etaVar*etaVar*(p012)*x*x*xym*t/(DeltaRa2*r0*r0*r0*r0*r0)
    -9*etaVar*etaVar*p0123*x*x*xym*t/(DeltaRa2*DeltaRa2*r0*r0*r0*r0)
    -4*etaVar*p012*x*x*t/(DeltaRa2*r0*r0*r0)
    - etaVar*p012*xym*t/(DeltaRa2*r0*r0*r0)
    +12*etaVar*p0123*x*x*t/(DeltaRa2*DeltaRa2*r0*r0)
    +3*etaVar*p0123*xym*t/(DeltaRa2*DeltaRa2*r0*r0)
    +2*p012*t/(DeltaRa2*r0)+ 9*etaVar*etaVar*p0123*x*x*xym*t*t/(2*DeltaRa2*r0*r0*r0*r0*r0)
    -6*etaVar*p0123*x*x*t*t/(DeltaRa2*r0*r0*r0)-3*etaVar*p0123*xym*t*t/(2*DeltaRa2*r0*r0*r0)
    +3*p0123*t*t/(DeltaRa2*r0);

    d2ydetaVar=3*p02*y*y*xym/(2*DeltaRa2*etaVar*etaVar*r0*r0*r0*r0*r0)
    - 3*p012*y*y*xym/(DeltaRa2*DeltaRa2*etaVar*etaVar*r0*r0*r0*r0)
    + 2*p02*y*y/(DeltaRa2*etaVar*etaVar*etaVar*r0*r0*r0)
    -p02*xym/(2*DeltaRa2*etaVar*r0*r0*r0)
    +3*p0123*y*y*xym/(DeltaRa2*DeltaRa2*DeltaRa2*etaVar*etaVar*r0*r0*r0)
    -4*p012*y*y/(DeltaRa2*DeltaRa2*etaVar*etaVar*etaVar*r0*r0)
    +p012*xym/(DeltaRa2*DeltaRa2*etaVar*r0*r0)
    -p02/(DeltaRa2*etaVar*etaVar*r0)
    +3*p012*y*y*xym*t/(DeltaRa2*etaVar*etaVar*r0*r0*r0*r0*r0)
    -9*p0123*y*y*xym*t/(DeltaRa2*DeltaRa2*etaVar*etaVar*r0*r0*r0*r0)
    +4*p012*y*y*t/(DeltaRa2*etaVar*etaVar*etaVar*r0*r0*r0) 
    -p012*xym*t/(DeltaRa2*etaVar*r0*r0*r0)
    -12*p0123*y*y*t/(DeltaRa2*DeltaRa2*etaVar*etaVar*etaVar*r0*r0)
    +3*p0123*xym*t/(DeltaRa2*DeltaRa2*etaVar*r0*r0)
    -2*p012*t/(DeltaRa2*etaVar*etaVar*r0)
    +9*p0123*y*y*xym*t*t/(2*DeltaRa2*etaVar*etaVar*r0*r0*r0*r0*r0)
    +6*p0123*y*y*t*t/(DeltaRa2*etaVar*etaVar*etaVar*r0*r0*r0)
    -3*p0123*xym*t*t/(2*DeltaRa2*etaVar*r0*r0*r0)
    -3*p0123*t*t/(DeltaRa2*etaVar*etaVar*r0)   ;
    //std::cout<<"Derivs after a2: "<<"x:"<<x<<"y:"<<y<<" etaVar: "<<etaVar<<" DeltaRa2: "<<DeltaRa2<<" d2ydetaVar: "<<d2ydetaVar<<std::endl;
    //std::cout<<" t: " <<t<<std::endl;
    //std::cout<<"p0: "<<p0<<" p1: "<<p1<<" p2: "<<p2<<" p3: "<<p3<<std::endl;

    //TEST DERIV begin ------------------
    /*
    real_type d=-(-p0/2.0+p2/2.0)*y*xym/(2*DeltaRa2*etaVar*r0*r0*r0)
    +(p0/2.0-p1+p2/2.0)*y*xym/(DeltaRa2*DeltaRa2*etaVar*r0*r0)
    -(-p0/2.0+p2/2.0)*y/(DeltaRa2*etaVar*etaVar*r0)
    -(p0/2.0-p1+p2/2.0)*y*xym*t/(DeltaRa2*etaVar*r0*r0*r0)
    +3*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*y*xym*t/(DeltaRa2*DeltaRa2*etaVar*r0*r0)
    ...
    double etaVar2=etaVar; //+0.0001;
    double y2=y+0.0001;
    r=std::sqrt(etaVar2*x*x+1.0/etaVar2*y2*y2);
    r0=std::sqrt(etaVar2*x*x+1.0/etaVar2*y2*y2);
    xym=x*x-y2*y2/(etaVar*etaVar);
    r*= DeltaRInva2;
    //std::cout<<" r2: "<<r<<std::endl;
    t     = std::modf(r, &ipart);
    i = (int)ipart;
 
    real_type dnew =-(-p0/2.0+p2/2.0)*y2*xym/(2*DeltaRa2*etaVar*r0*r0*r0)
    +(p0/2.0-p1+p2/2.0)*y2*xym/(DeltaRa2*DeltaRa2*etaVar*r0*r0)
    -(-p0/2.0+p2/2.0)*y2/(DeltaRa2*etaVar*etaVar*r0)
    -(p0/2.0-p1+p2/2.0)*y2*xym*t/(DeltaRa2*etaVar*r0*r0*r0)
    +3*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*y2*xym*t/(DeltaRa2*DeltaRa2*etaVar*r0*r0)
    -2*(p0/2.0-p1+p2/2.0)*y2*t/(DeltaRa2*etaVar*etaVar*r0)
    -3*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*y2*xym*t*t/(2*DeltaRa2*etaVar*r0*r0*r0)
    -3*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*y2*t*t/(DeltaRa2*etaVar*etaVar*r0);

    //std::cout<<"0: "<<coefs[shift_index+i+0]<<" 1: "<<coefs[shift_index+i+1]<<" 2: "<<coefs[shift_index+i+2]<<" 3: "<<coefs[shift_index+i+3]<<std::endl;

    std::cout<<" d2ydetaVar approx: "<<(dnew-d)/0.0001<<std::endl;
    //std::cout<<" t2: "<<t<<std::endl;
    */
    //TEST DERIV end ----------------------------

    //-------------------------------
    //std::cout<<"Derivs after a2: "<<"x:"<<x<<"y:"<<y<<" etaVar: "<<etaVar<<" DeltaRa2: "<<DeltaRa2<<" derivs[..]: "<<derivs[NumParamsu+NumParamsa2+NumParamsb2][0]<<std::endl;
    //std::cout<<"p0: "<<p0<<" p1: "<<p1<<" p2: "<<p2<<" p3: "<<p3<<std::endl;
    //std::cout<<"t: "<<tp[2]<<std::endl;
    ...
    tp[2] = t;
    tp[3] = 1.0;
    shift_index=NumParamsu+3+NumParamsa2+3;  //+3 or not??
    //i=i+shift_index;
    p0 = coefs[shift_index + i + 0];
    p1 = coefs[shift_index + i + 1];
    p2 = coefs[shift_index + i + 2];
    ...
    for (int n = imin; n < imax; ++n)
      derivs[n +NumParamsu+NumParamsa2] = SplineDerivs[n]; 
    ...
    //std::cout<<"b2 deriv: "<<derivs[NumParamsu+NumParamsa2+NumParamsb2][0]<<std::endl;
   
    xym=y*y-x*x/(etaVar*etaVar);


    dydetaVar= dydetaVar -etaVar*(-p0/2.0+p2/2.0)*y*(xym)/(2*DeltaRb2*r0*r0*r0)
    +etaVar*(p0/2.0-p1+p2/2.0)*y*xym/(DeltaRb2*DeltaRb2*r0*r0)+(-p0/2.0+p2/2.0)*y/(DeltaRb2*r0)
    -etaVar*(p0/2.0-p1+p2/2.0)*y*xym*t/(DeltaRb2*r0*r0*r0)
    +3*etaVar*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*y*xym*t/(DeltaRb2*DeltaRb2*r0*r0)
    +2*(p0/2.0-p1+p2/2.0)*y*t/(DeltaRb2*r0)-3*etaVar*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*y*xym*t*t/(2*DeltaRb2*r0*r0*r0)
    +3*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*y*t*t/(DeltaRb2*r0);

    
    dxdetaVar= dxdetaVar -(-p0/2.0+p2/2.0)*x*xym/(2*DeltaRb2*etaVar*r0*r0*r0)
    +(p0/2.0-p1+p2/2.0)*x*xym/(DeltaRb2*DeltaRb2*etaVar*r0*r0)
    -(-p0/2.0+p2/2.0)*x/(DeltaRb2*etaVar*etaVar*r0)
    -(p0/2.0-p1+p2/2.0)*x*xym*t/(DeltaRb2*etaVar*r0*r0*r0)
    +3*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*x*xym*t/(DeltaRb2*DeltaRb2*etaVar*r0*r0)
    -2*(p0/2.0-p1+p2/2.0)*x*t/(DeltaRb2*etaVar*etaVar*r0)
    -3*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*x*xym*t*t/(2*DeltaRb2*etaVar*r0*r0*r0)
    -3*(-p0/6.0+p1/2.0-p2/2.0+p3/6.0)*x*t*t/(DeltaRb2*etaVar*etaVar*r0);
    ...
    d2ydetaVar=d2ydetaVar+ 3*etaVar*etaVar*(p02)*y*y*xym/(2*DeltaRb2*r0*r0*r0*r0*r0)
    -3*etaVar*etaVar*(p012)*y*y*xym/(DeltaRb2*DeltaRb2*r0*r0*r0*r0)
    -2*etaVar*(p02)*y*y/(DeltaRb2*r0*r0*r0) 
    - etaVar*(p02)*xym/(2*DeltaRb2*r0*r0*r0)
    + 3*etaVar*etaVar*(p0123)*y*y*xym/(DeltaRb2*DeltaRb2*DeltaRb2*r0*r0*r0)
    + 4*etaVar*(p012)*y*y/(DeltaRa2*DeltaRb2*r0*r0)
    +etaVar*(p012)*xym/(DeltaRb2*DeltaRb2*r0*r0)
    +(p02)/(DeltaRb2*r0)
    +3*etaVar*etaVar*(p012)*y*y*xym*t/(DeltaRb2*r0*r0*r0*r0*r0)
    -9*etaVar*etaVar*p0123*y*y*xym*t/(DeltaRb2*DeltaRb2*r0*r0*r0*r0)
    -4*etaVar*p012*y*y*t/(DeltaRb2*r0*r0*r0)
    - etaVar*p012*xym*t/(DeltaRb2*r0*r0*r0)
    +12*etaVar*p0123*y*y*t/(DeltaRb2*DeltaRb2*r0*r0)
    +3*etaVar*p0123*xym*t/(DeltaRb2*DeltaRb2*r0*r0)
    +2*p012*t/(DeltaRb2*r0)+ 9*etaVar*etaVar*p0123*y*y*xym*t*t/(2*DeltaRb2*r0*r0*r0*r0*r0)
    -6*etaVar*p0123*y*y*t*t/(DeltaRb2*r0*r0*r0)-3*etaVar*p0123*xym*t*t/(2*DeltaRb2*r0*r0*r0)
    +3*p0123*t*t/(DeltaRb2*r0);

    d2xdetaVar=d2xdetaVar+  3*p02*x*x*xym/(2*DeltaRb2*etaVar*etaVar*r0*r0*r0*r0*r0)
    - 3*p012*x*x*xym/(DeltaRb2*DeltaRb2*etaVar*etaVar*r0*r0*r0*r0)
    + 2*p02*x*x/(DeltaRb2*etaVar*etaVar*etaVar*r0*r0*r0)
    -p02*xym/(2*DeltaRb2*etaVar*r0*r0*r0)
    +3*p0123*x*x*xym/(DeltaRb2*DeltaRb2*DeltaRb2*etaVar*etaVar*r0*r0*r0)
    -4*p012*x*x/(DeltaRb2*DeltaRb2*etaVar*etaVar*etaVar*r0*r0)
    +p012*xym/(DeltaRb2*DeltaRb2*etaVar*r0*r0)
    -p02/(DeltaRb2*etaVar*etaVar*r0)
    +3*p012*x*x*xym*t/(DeltaRb2*etaVar*etaVar*r0*r0*r0*r0*r0)
    -9*p0123*x*x*xym*t/(DeltaRb2*DeltaRb2*etaVar*etaVar*r0*r0*r0*r0)
    +4*p012*x*x*t/(DeltaRb2*etaVar*etaVar*etaVar*r0*r0*r0) 
    -p012*xym*t/(DeltaRb2*etaVar*r0*r0*r0)
    -12*p0123*x*x*t/(DeltaRb2*DeltaRb2*etaVar*etaVar*etaVar*r0*r0)
    +3*p0123*xym*t/(DeltaRb2*DeltaRb2*etaVar*r0*r0)
    -2*p012*t/(DeltaRb2*etaVar*etaVar*r0)
    +9*p0123*x*x*xym*t*t/(2*DeltaRb2*etaVar*etaVar*r0*r0*r0*r0*r0)
    +6*p0123*x*x*t*t/(DeltaRb2*etaVar*etaVar*etaVar*r0*r0*r0)
    -3*p0123*xym*t*t/(2*DeltaRb2*etaVar*r0*r0*r0)
    -3*p0123*t*t/(DeltaRb2*etaVar*etaVar*r0)   ;
    //std::cout<<" b2 d2xdeta: "<<d2XdetaVarb2<<std::endl;

   

    //for (int n = 0; n <NumParamsu+NumParamsa2+NumParamsb2 ; ++n)
    //  std::cout<<" derivs: "<<n<<" : "<<derivs[n] <<std::endl;
    return true;
  }
  ...
  inline real_type f(real_type r, real_type xsquared, real_type ysquared, int numpart)  override
  {
    if (r >= cutoff_radius)
      return 0.0;
    return evaluate(r, xsquared, ysquared, numpart);
  }
...
inline real_type df(real_type r, real_type xsquared, real_type ysquared, int numpart) override
  {
    if (r >= cutoff_radius)
      return 0.0;
    real_type du, d2u;
    evaluate(r, xsquared, ysquared, numpart, du, d2u);
    return du;
  }
              ...
              T* restrict displArrayCompressedYsquared) const
{
  ...
  if (numpar==0){
    ...
    shift_index=NumParamsu+3+NumParamsa2+3;} 
  //std::cout<<"AV in evaluateV2, numpar: "<<numpar<<"shift_index: "<<shift_index<<std::endl;
