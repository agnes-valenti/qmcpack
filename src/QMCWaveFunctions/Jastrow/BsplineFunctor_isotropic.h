...
struct BsplineFunctor : public OptimizableFunctorBase
{
  ...
  {
    NumParams    = n;
    int numCoefs = NumParams + 3; //4;  //AV changed for without cusp!
    int numKnots = numCoefs - 2 ;  //- 2;  //AV changed for without cusp!
  ...
  void reset() override
  {
    const int numCoefs = NumParams+3;  // + 4;  //AV changed for without cusp!
    const int numKnots = numCoefs -2; // - 2;  //AV changed for without cusp!
    DeltaR             = cutoff_radius / (real_type)(numKnots - 1);
    DeltaRInv          = 1.0 / DeltaR;
  ...
  inline bool evaluateDerivatives(real_type r, std::vector<TinyVector<real_type, 3>>& derivs) override
  {
    ...
    for (int n = imin; n < imax; ++n)
      derivs[n ] = SplineDerivs[n];  //derivs[n - 1] = SplineDerivs[n]; //AV change because of cusp condition
