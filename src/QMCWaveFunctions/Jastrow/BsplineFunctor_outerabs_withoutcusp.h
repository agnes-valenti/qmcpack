...
struct BsplineFunctor : public OptimizableFunctorBase
{
  ...
  void resize(int n)
  {
    ...
    int numCoefs = (NumParamsu + 3+3*Nv); //4;  //AV changed for without cusp!
  ...
  void reset() override
  {
    ...
    //std::cout<<"etaVar: "<<etaVar<<std::endl;
    const int numCoefs = NumParamsu+3+3*Nv;  // + 4;  //AV changed for without cusp!
    ...
    */
    ...
    for (int i = 0; i < Nv; i++)
      coefs[NumParamsu+3+Nv+i] = Parameters[NumParamsu+Nv+i];

    for (int i = 0; i < Nv; i++)
      coefs[NumParamsu+3+Nv+Nv+i] = Parameters[NumParamsu+Nv+Nv+i];
...
void evaluateVGL2(const int iat,
  ...
  { 
    ...
    int shift_index_c=nparamsu+3;
    int shift_index_alpha=nparamsu+3+nv;
    int shift_index_beta=nparamsu+3+nv+nv;
    ...
    if (tauvalue>1.5){
      shift_index_alpha=nparamsu+3+nv+nv;
      shift_index_beta=nparamsu+3+nv;
