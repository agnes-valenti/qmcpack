  ...
  { 
    ...
    */
    ...
    if (optimizeCAlphaBeta){
      for (int i_nv=0; i_nv<nv; i_nv++){
        real_type r_an=coefs[shift_index_alpha+i_nv]* coefs[shift_index_alpha+i_nv]*fx*fx+coefs[shift_index_beta+i_nv]* coefs[shift_index_beta+i_nv]*fy*fy;
        if (std::abs(r_an)<1e-15){
          r_an+=1e-15;
        }
        //real_type vderiv=std::pow(r_an,((i_nv+1)/2.0));
        derivs[deriv_shift_c+i_nv][0]=std::pow(Lx,-(i_nv+1.0))*std::pow(r_an,((i_nv+1)/2.0));
        derivs[deriv_shift_alpha+i_nv][0]=std::pow(Lx,-(i_nv+1.0))*(i_nv+1)/2.0*coefs[shift_index_c+i_nv]*std::pow(r_an,((i_nv+1)/2.0-1.0))*fx*fx*2*coefs[shift_index_alpha+i_nv];
        derivs[deriv_shift_beta+i_nv][0]=std::pow(Lx,-(i_nv+1.0))*(i_nv+1)/2.0*coefs[shift_index_c+i_nv]*std::pow(r_an,((i_nv+1)/2.0-1.0))*fy*fy*2*coefs[shift_index_beta+i_nv];
...
#pragma omp simd reduction(+ : d)
  for (int jat = 0; jat < iCount; jat++)
  {
    ...
    for (int i_nv=0; i_nv<nv; i_nv++){
      double r_an=coefs[shift_index_alpha+i_nv]*coefs[shift_index_alpha+i_nv]*fx*fx+coefs[shift_index_beta+i_nv]*coefs[shift_index_beta+i_nv]*fy*fy;
      if (std::abs(r_an)<1e-15){
        r_an+=1e-15;
      }
      vsum+=std::pow(Lx,-(i_nv+1.0))*coefs[shift_index_c+i_nv]*std::pow(r_an,((i_nv+1)/2.0));
  ...
  for (int j = 0; j < iCount; j++)
  {
    ...
    */
    ...
    for (int i_nv=0; i_nv<nv; i_nv++){
      ...
      vsum+=std::pow(Lx,-(i_nv+1.0))*coefs[shift_index_c+i_nv]*std::pow(r_an,((i_nv+1)/2.0));
      vxderiv+=std::pow(Lx,-(i_nv+1.0))*coefs[shift_index_c+i_nv]*n/2.0*std::pow(r_an,(n/2.0-1))*(2.0*coefs[shift_index_alpha+i_nv]*coefs[shift_index_alpha+i_nv]*fx*dfxdx);
      
      vyderiv+=std::pow(Lx,-(i_nv+1.0))*coefs[shift_index_c+i_nv]*n/2.0*std::pow(r_an,(n/2.0-1))*(2.0*coefs[shift_index_beta+i_nv]*coefs[shift_index_beta+i_nv]*fy*dfydy);
      
      vx2deriv+=std::pow(Lx,-(i_nv+1.0))*coefs[shift_index_c+i_nv]*n/2.0*((n/2.0-1)*std::pow(r_an,(n/2.0-2))*(2.0*coefs[shift_index_alpha+i_nv]*coefs[shift_index_alpha+i_nv]*fx*dfxdx)*(2.0*coefs[shift_index_alpha+i_nv]*coefs[shift_index_alpha+i_nv]*fx*dfxdx)
        + std::pow(r_an,(n/2.0-1))*(2.0*coefs[shift_index_alpha+i_nv]* coefs[shift_index_alpha+i_nv]*(dfxdx*dfxdx+fx*d2fxdx2))); 

      vy2deriv+=std::pow(Lx,-(i_nv+1.0))*coefs[shift_index_c+i_nv]*n/2.0*((n/2.0-1)*std::pow(r_an,(n/2.0-2))*(2.0*coefs[shift_index_beta+i_nv]* coefs[shift_index_beta+i_nv]*fy*dfydy)*(2.0*coefs[shift_index_beta+i_nv]*coefs[shift_index_beta+i_nv]*fy*dfydy)
        + std::pow(r_an,(n/2.0-1))*(2.0*coefs[shift_index_beta+i_nv]*coefs[shift_index_beta+i_nv]* (dfydy*dfydy+fy*d2fydy2))); 
